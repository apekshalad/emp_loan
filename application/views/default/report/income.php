				...
				<div class="box box-primary">
					...
					echo form_open_multipart('report/income',$attributes); ?>
							...
							<div class="row">
            					...
            					<div class="col-md-3">
            						<div class="form-group" >
            						  <label for="timePeriods">Select Clients</label><br />
            						  <select id="clients_id" name="clients_id" class="form-control select2 input-sm">
            								<option value="">Select Clients</option>
                							<?php 
                                            $sqlPro1=$this->db->select("*")->from("clients")->get()->result();
                            				foreach($sqlPro1 as $sqlPro2)
                                            {
                                                $selected = '';
                                                if($sqlPro2->client_ID == $_REQUEST['clients_id'])
                                                {
                                                    $selected = 'selected="selected"';
                                                }
                            					?>
                    							<option value="<?php echo $sqlPro2->client_ID?>" <?php echo $selected;?>><?php echo $sqlPro2->client_name?></option>
                                                <?php 
                                            }?>
            						  </select>
					...
					{		
						...
						{ 
							...
							<tr>
                              ...
                              <td>
                                <?php 
                                $options = array('user_id'=>$row->user_id,'meta_key'=>'acc_no');
                                $res = $this->db->get_where('user_meta' , $options)->row();
                                echo $res->meta_value;
                                ?>
                              </td>
