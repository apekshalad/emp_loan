...
class Expense extends MY_Controller {	
	...
	public function manage()
	{
		...
		$query = "SELECT * FROM `expense_category` order by id DESC";
        //echo $query;
		$data['query'] = $this->Common_model->commonQuery($query);
		
		$data['theme']=$theme;
		
		$data['content'] = "$theme/cat_expense/manage";
	...
	public function add_new()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			if ($this->form_validation->run() != FALSE)
			{
				...
				$branch_id=$this->Common_model->commonInsert('expense_category',$datai);
				...
				$_SESSION['msg'] = '<div class="alert alert-success alert-dismissable" style="margin-bottom:0px;">
							...
							';
				redirect('/expense/manage','location');	
		...
		$data['theme']=$theme;
		$data['content'] = "$theme/cat_expense/add_new";
		$this->load->view("$theme/header",$data);
		//$data['sidebar'] = 'sidebar-left';		
