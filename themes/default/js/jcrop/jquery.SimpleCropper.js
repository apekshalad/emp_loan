...
(function ($) {
    ...
    $.fn.simpleCropper = function (onComplete) {
        ...
        var jcrop_api;
		var xhr;
        var bottom_html = "<input type='file' value='' id='fileInput' name='files[]'/ accept='image/*'><canvas id='myCanvas' style='display:none;'></canvas><div id='modal'></div><div id='preview'><div class='buttons'><div class='cancel'>Cancel</div><div class='ok'>Crop</div></div></div>";
        ...
        function preview() {
			...
			/*ajax callback to decode image*/
			$('.full_sreeen_overlay').show();
			
			$.ajax({
				url: base_url+'index.php/ajax/decode_image',
				type: 'POST',
				data: {
					encode_img_string: dataUrl,
				...
				{
					alert('Unknown Error Occured While Uploading Image.');
				}
			});
			/*end of ajax callback to decode image*/
